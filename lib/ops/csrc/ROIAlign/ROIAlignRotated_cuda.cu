...
template <typename T>
__device__ T nearest_interpolate(
    const T* input,
    const int height,
    const int width,
    T y,
  ...
  int x_low = round(x), y_low = round(y);

  // do nearest interpolation
  T val = input[y_low * width + x_low];

  return val;
}

template <typename T>
__device__ void nearest_interpolate_gradient(
    const int height,
    const int width,
    T y,
    T x,
  ...
  x_low = (int)round(x), y_low = (int)round(y);
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) // e.g., iy = 0, 1
    {
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        if (interpolation_method == 0) {
          val = bilinear_interpolate(offset_input, height, width, y, x);
        } else if (interpolation_method == 1) {
          val = nearest_interpolate(offset_input, height, width, y, x);
        }
        output_val += val;
      }
    }
...
__global__ void RoIAlignRotatedBackwardFeature(
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    if (!aligned) { // for backward-compatibility only
      roi_width = max(roi_width, (T)1.0);
      roi_height = max(roi_height, (T)1.0);
    }
    T bin_size_h = static_cast<T>(roi_height) / static_cast<T>(pooled_height);
    T bin_size_w = static_cast<T>(roi_width) / static_cast<T>(pooled_width);
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) // e.g., iy = 0, 1
    {
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        if (interpolation_method == 0) {
          ...
          nearest_interpolate_gradient(
            height, width, y, x, x_low, y_low);

          T g = top_diff_this_bin / count;

          if (x_low >= 0 && y_low >= 0) {
            atomicAdd(
              offset_bottom_diff + y_low * width + x_low, static_cast<T>(g));
          } // if
