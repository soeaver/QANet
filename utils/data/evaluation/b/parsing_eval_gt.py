...
def parsing_png_with_rescoring(parsing, scores, semseg_thresh, img_info, output_folder, semseg=None):
    ...
    new_score = copy.deepcopy(scores)

    im_name = img_info['file_name']

    gt_pth = '/home/user/Program/Pet-dev/data/CIHP/val_parsing/' + im_name.replace('.jpg', '*')
    # gt_pth = '/home/user/Program/Pet-dev/data/MHP-v2/val_parsing/' + im_name.replace('.jpg', '*')
    gt_list = glob.glob(gt_pth)
    ...
    for k in range(len(_inx)):
        mask_pred = parsing[_inx[k]].astype(np.int)
        ovmax = -np.inf
        for _ in gt_list:
            mask_gt = cv2.imread(_, 0)
            seg_iou = cal_one_mean_iou(mask_pred.astype(np.uint8), mask_gt, 20)
            mean_seg_iou = np.nanmean(seg_iou)
            if mean_seg_iou > ovmax:
                ovmax = mean_seg_iou
        new_score[_inx[k]] = np.sqrt(ovmax * scores[_inx[k]])

        if new_score[_inx[k]] < semseg_thresh:
            continue
        ...
        ins_id += 1
        ins_pair.append((new_score[_inx[k]], scores[_inx[k]]))
...
def parsing_png(parsing, scores, semseg_thresh, img_info, output_folder):
    parsing_output_dir = os.path.join(output_folder, 'parsing_predict')
    if not os.path.exists(parsing_output_dir):
        os.makedirs(parsing_output_dir)
    parsing_ins_dir = os.path.join(output_folder, 'parsing_instance')
...
def evaluate_parsing(all_results, eval_ap, score_thresh, num_parsing, im_dir, ann_fn, output_folder):
    ...
    _iou, _miou = parsing_iou(predict_dir, im_dir, num_parsing)
