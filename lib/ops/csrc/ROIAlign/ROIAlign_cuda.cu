...
__device__ T nearest_interpolate(
  ...
  T val = bottom_data[y_low * width + x_low];
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) // e.g., iy = 0, 1
    {
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        if (interpolation_method == 0) {
          val = bilinear_interpolate(offset_bottom_data, height, width, y, x, index);
        } else if (interpolation_method == 1) {
          val = nearest_interpolate(offset_bottom_data, height, width, y, x, index);
        }
        output_val += val;
      }
    }
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        if (interpolation_method == 0) {
          ...
          nearest_interpolate_gradient(
            height, width, y, x, x_low, y_low, index);
