...
void soft_nms_cpu_kernel(
  ...
  T ix1, iy1, ix2, iy2, iscore, iarea, max_score, inter, ovr;
  for (int i = 0, i_box = 0; i < ndets; i++, i_box += 4) {
    // get max box
    max_pos = i;
    max_score = scores[i];
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> soft_nms_cpu(
  ...
  return std::make_tuple(res_dets, res_scores, res_inds);
}
