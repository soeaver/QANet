...
class UVOutputs(nn.Module):
    def __init__(self, cfg, dim_in, spatial_scale):
        ...
        self.deconv_Ann = nn.ConvTranspose2d(dim_in, num_parts + 1, kernel_size=4, stride=2, padding=1)
        self.deconv_Index = nn.ConvTranspose2d(dim_in, num_patches + 1, kernel_size=4, stride=2, padding=1)
        self.deconv_U = nn.ConvTranspose2d(dim_in, num_patches + 1, kernel_size=4, stride=2, padding=1)
        self.deconv_V = nn.ConvTranspose2d(dim_in, num_patches + 1, kernel_size=4, stride=2, padding=1)
