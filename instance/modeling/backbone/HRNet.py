...
class HRNet(hr.HRNet):
    def __init__(self, norm='bn', stride=32):
        ...
        self.stage4 = nn.Sequential(
            ...
            hr.StageModule(block_2, base_width, norm, stage_with_conv[3], use_se, self.use_global, 4, 4),
            hr.StageModule(block_2, base_width, norm, stage_with_conv[3], use_se, self.use_global, 4, multi_out),
...
def hrnet_s4():
    model = HRNet(norm=get_norm(), stride=4)
    return model


@registry.BACKBONES.register("hrnet")
def hrnet():
    model = HRNet(norm=get_norm(), stride=32)
    return model
