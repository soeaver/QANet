...
class ProbOhemCrossEntropy2d(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.min_kept > num_valid:
            logging_rank('Labels: {}'.format(num_valid))
        elif num_valid > 0:
            prob = prob.masked_fill_((1 - valid_mask.long()).bool(), 1)
            mask_prob = prob[
            ...
            if self.min_kept > 0:
                ...
                target *= kept_mask.long()
                valid_mask *= kept_mask
                # logging_rank('Valid Mask: {}'.format(valid_mask.sum()))

        target = target.masked_fill_((1 - valid_mask.long()).bool(), self.ignore_label)
