...
class SemanticSegmentation(object):
    ...
    def __init__(self, semseg, class_ids, size, mode='poly', extra_fields=None):
        ...
        self.ignore_label = self.extra_fields['ignore_label'] if 'ignore_label' in self.extra_fields else 0
        self.label_shift = self.extra_fields['label_shift'] if 'label_shift' in extra_fields else 1
...
def convert_pano_to_semseg(image_path, extra_fileds, name):
    ...
    coco format. It supports two main stream process way, get the all
    things ids to one or not, which is using 'extra_field['convert_format']'
    to control.
    """
    from pet.lib.datasets.evaluation.panoptic_eval import rgb2id
