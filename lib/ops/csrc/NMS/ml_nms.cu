...
__device__ inline bool devIoU(
    int64_t const a_label,
    T const* const a,
    T const* const b,
    const float threshold) {
  if (a_label != b[4]) return false;
...
__global__ void ml_nms_kernel(
  ...
  if (threadIdx.x < col_size) {
    ...
    block_boxes[threadIdx.x * 5 + 4] =
        dev_labels[threadsPerBlock * col_start + threadIdx.x];
  }
  __syncthreads();

  if (threadIdx.x < row_size) {
    const int cur_box_idx = threadsPerBlock * row_start + threadIdx.x;
    const int64_t cur_label = dev_labels[cur_box_idx];
    ...
    for (i = start; i < col_size; i++) {
      if (devIoU<T>(cur_label, cur_box, block_boxes + i * 5, iou_threshold)) {
        t |= 1ULL << i;
      }
    }
...
at::Tensor ml_nms_cuda(
  ...
  auto labels_sorted = labels.index_select(0, order_t).contiguous();
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      dets_sorted.scalar_type(), "ml_nms_kernel_cuda", [&] {
        ml_nms_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
            dets_num,
            iou_threshold,
            dets_sorted.data_ptr<scalar_t>(),
            labels_sorted.data_ptr<int64_t>(),
