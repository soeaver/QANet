...
@registry.QANET_HEADS.register("gce_head")
class gce_head(nn.Module):
    def __init__(self, dim_in):
        super(gce_head, self).__init__()
        self.dim_in = dim_in[-1]

        use_nl = cfg.QANET.GCE_HEAD.USE_NL
        use_bn = cfg.QANET.GCE_HEAD.USE_BN
        use_gn = cfg.QANET.GCE_HEAD.USE_GN
        conv_dim = cfg.QANET.GCE_HEAD.CONV_DIM
        asppv3_dim = cfg.QANET.GCE_HEAD.ASPPV3_DIM
        num_convs_before_asppv3 = cfg.QANET.GCE_HEAD.NUM_CONVS_BEFORE_ASPPV3
        asppv3_dilation = cfg.QANET.GCE_HEAD.ASPPV3_DILATION
        num_convs_after_asppv3 = cfg.QANET.GCE_HEAD.NUM_CONVS_AFTER_ASPPV3
        ...
        if use_nl:
            feat_list.append(
                NonLocal2d(conv_dim, int(conv_dim * cfg.QANET.GCE_HEAD.NL_RATIO), conv_dim, use_gn=True)
            )
