...
template <typename T>
void pre_calc_for_nearest_interpolate(
    const int height,
    const int width,
...
template <typename T>
void nearest_interpolate_gradient(
    const int height,
    const int width,
    T y,
    T x,
...
void ROIAlignRotatedForward(
  ...
  for (int n = 0; n < n_rois; n++) {
    ...
    if (interpolation_method == 0) {
      ...
      pre_calc_for_nearest_interpolate(
          height,
          width,
          pooled_height,
          pooled_width,
          ...
          for (int iy = 0; iy < roi_bin_grid_h; iy++) {
            for (int ix = 0; ix < roi_bin_grid_w; ix++) {
              ...
              if (interpolation_method == 0) {
                ...
                if (pc.pos1 >=0 ) {
                  output_val += offset_input[pc.pos1];
                }
              }
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        if (interpolation_method == 0) {
          ...
          T g = grad_output_this_bin / count;

          if (x_low >= 0 && y_low >= 0) {
            add(offset_grad_input + y_low * width + x_low, static_cast<T>(g));
          } // if
