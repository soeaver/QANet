...
class ResNeXt(nn.Module):
    def __init__(self, aligned=False, use_3x3x3stem=False, avg_down=False, stem_width=64, base_width=4, cardinality=32,
                 layers=(3, 4, 23, 3),
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, conv='Conv2d', ctx=''):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, self.base_width, self.cardinality, stride, dilation, downsample=downsample,
                  stride_3x3=True, conv=conv, norm=self.norm, ctx=ctx)
        )
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, self.base_width, self.cardinality, 1, dilation, downsample=None,
                      stride_3x3=True, conv=conv, norm=self.norm, ctx=ctx)
            )
