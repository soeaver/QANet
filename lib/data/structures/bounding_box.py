...
class BoxList(object):
    ...
    def move(self, gap):
        ...
        moved_bbox = torch.cat((moved_xmin, moved_ymin, moved_xmax, moved_ymax), dim=-1)
    ...
    def transpose(self, method):
        ...
        if len(BoxList.FLIP_MAP) > 0:
            labels = self.get_field('labels')
            for i in BoxList.FLIP_MAP:
                left = torch.where(labels == i[0])[0]
                right = torch.where(labels == i[1])[0]
                labels[left] = i[1]
                labels[right] = i[0]
    ...
    def random_select(self, fraction=-1):
        """Random select a set of boxes"""
        ...
        if fraction > 0:
            assert 0 < fraction < 1
            l = self.bbox.shape[0]
            keep_inds = np.random.choice(l, max(int(l * fraction), 1), replace=False)
            bbox[keep_inds]
