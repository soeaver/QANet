...
from lib.ops.boxlist_ops import remove_boxes_by_center, remove_boxes_by_overlap
...
class RandomCrop(object):
    def __init__(self, crop_sizes, pad_pixel=(0, 0, 0), iou_ths=(0.7,), mode='bgr255', boarder=-1, cat_max_ratio=1, ignore_label=255):
        self.crop_sizes = crop_sizes
        self.pad_pixel = pad_pixel
        self.iou_ths = iou_ths
        self.boarder = boarder  # for CenterNet
        self.cat_max_ratio = cat_max_ratio
        self.ignore_label = ignore_label
        self.mode = mode

    def get_boarder(self, size):
        i = 1
        size /= 2
        while size <= self.boarder // i:
            i *= 2
        return self.boarder // i

    def get_crop_coordinate(self, image_size):
        w, h = image_size
        crop_h, crop_w = random.choice(self.crop_sizes)

        w_boarder = (crop_w / 2) if self.boarder < 0 else self.get_boarder(w)
        h_boarder = (crop_h / 2) if self.boarder < 0 else self.get_boarder(h)
        left = random.randint(*sorted(map(int, (w_boarder, w - w_boarder)))) - crop_w // 2
        up = random.randint(*sorted(map(int, (h_boarder, h - h_boarder)))) - crop_h // 2
...
class CropAndExpand(object):
    def __init__(self, min_ious=(0.1, 0.3, 0.5, 0.7, 0.9), min_crop_size=0.3,
                 ratio_range=(1, 4), prob=0.5, pad_pixel=(0, 0, 0), mode="bgr255"):
        # 1: return ori img
